...
- **Client (DistributionHub)**: `activePlatformCount` uses `isComingSoonPlatform()` from `usePlatformAccess()` hook (not hardcoded `supported` field). Button text uses `formatPlatformList()` for dynamic copy ("Posting to TikTok & YouTube").
