...
**Responsive grids:** Browse clip grid uses `repeat(auto-fill, minmax(300px, 1fr))` — columns adapt to available width, never below 300px per card (2 cols on portrait tablets, 3-5 on desktop). Grid cell wrappers have `overflow-hidden` to contain legendary gem ornaments. CTA buttons hardened: `whitespace-nowrap overflow-hidden text-ellipsis` on label text (leg-cta, cta-viral, tp-cta-compact) — text never wraps or overflows. Other grids: affiliate stats `grid-cols-2 sm:grid-cols-4`, settings referral `grid-cols-1 sm:grid-cols-3`, caption styles `grid-cols-2 sm:grid-cols-3`, split-screen framing `grid-cols-2 sm:grid-cols-3`. Feed tabs: `overflow-x-auto` with hidden scrollbar + fade mask. Daily Radar: `flex-wrap min-h-14 py-2`.
