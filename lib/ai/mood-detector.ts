...
const SYSTEM_PROMPT = `You are a clip mood analyzer for a viral video editing app. Analyze the given clip transcript, title, and context to determine the dominant mood.
...
- If the transcript is empty or unclear, default to "hype" with low confidence`
...
function parseMoodResponse(text: string): MoodDetectionResult {
  try {
    ...
    return { mood, confidence, explanation, secondary_mood: secondaryMood }
  } catch {
