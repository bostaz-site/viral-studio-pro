...
import { useCreatePipeline } from '@/lib/hooks/useCreatePipeline'
import { useRemixPipeline } from '@/lib/hooks/useRemixPipeline'
import { pollClipRendering } from '@/lib/hooks/useVideoPolling'
...
function CreatePage() {
  ...
  const { processingStep, setProcessingStep, uploadProgress, errorMessage, reset: resetVideo } = useVideoStore()
  const { generatedClips, setGeneratedClips, clearClips } = useClipsStore()

  const { runPipeline } = useCreatePipeline({ pendingFile, url, srtFile, setVideoUrl })
  const { runRemixPipeline, loadExistingVideo } = useRemixPipeline(setVideoUrl)
