# SYSTEM REFERENCE — Admin Scraper & Discovery Layer (v1)
...
| `app/(dashboard)/admin/scraper/_components/youtube-scraper-form.tsx` | Formulaire recherche YouTube + saved searches |
| `app/(dashboard)/admin/scraper/_components/discovery-results-table.tsx` | Table resultats — select, import bulk, status badges |
| `app/(dashboard)/admin/scraper/_components/quota-panel.tsx` | Barre quota YouTube API (10k units/jour) |
| `app/api/admin/scraper/youtube/route.ts` | POST search + GET results par run_id |
...
| `lib/admin/scraper/youtube.ts` | YouTube Data API v3 client (search + channel details + email extraction) |
| `lib/admin/scraper/keyword-scorer.ts` | Keyword pre-score — regex affiliate signals (no AI) |
...
2. POST /api/admin/scraper/youtube
   ...
   e. For each channel:
      - Extract emails from description (regex + provenance)
      - Keyword pre-score (regex: "use code", "affiliate", "#ad", etc.)
      - Distributor graph detection (OpusClip, Submagic, etc.)
      - INSERT lead_discovery_results
...
- Search: 100 units per call (max 50 results)
- Channel details: 1 unit per call (up to 50 IDs batched)
- Strategy: ~100 searches/day = ~5,000 leads/day at cost of ~10,100 units
...
- Save email without source_url (NO source = NO contact)
...
- Forget is_business_contact=true for YouTube About emails
