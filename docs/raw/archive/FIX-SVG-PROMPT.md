# FIX URGENT — SVGs Browse Redesign

Les SVGs DiamondCorner, MasterCorner et MasterCrown dans `components/trending/rank-badge.tsx` sont des versions simplifiées qui ne matchent pas le mockup. Il faut les remplacer par les SVGs originaux du mockup.

## 1. DiamondCorner — REMPLACER COMPLÈTEMENT

Le SVG actuel est un gros octogone 44x44 qui crée des blobs dorés. Le vrai est un diamant facetté viewBox 32x32 avec glow radial et facettes brillantes.
...
## 4. Import React pour useId

Ajouter `import React from 'react'` en haut de rank-badge.tsx si pas déjà là.

## IMPORTANT
- Ne touche à RIEN d'autre que les 3 SVG components + l'import React
- Les SVGs dans le CSS (corners sizing, positioning) sont déjà corrects
- Le reste du redesign (scores, CTA, frames CSS, animations) est bon
